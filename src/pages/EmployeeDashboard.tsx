...
const Dashboard = () => {
    ...
    const { data: applied_jobs, isLoading: isLoadingAppliedJobs } = useGetMyApplicationsQuery(currentUser?.id);
            ...
            <section className='px-[40px]'>
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="bg-white py-4 px-8 lg:h-[150px] rounded-lg shadow-md flex justify-between items-center">
                        <div>
                            <p className="text-5xl font-bold text-blue-700">{isLoading ? 0 : saved_jobs?.data.length}</p>
                    ...
                    <div className="bg-white py-4 px-8 lg:h-[150px] rounded-lg shadow-md flex justify-between items-center">
                        <div>
                            <p className="text-5xl font-bold text-blue-700">{isLoadingAppliedJobs ? 0 : applied_jobs?.data.length}</p>
                ...
                <div className="mt-10">
                    ...
                    <div className='mt-4'>
                        {saved_jobs?.data.length === 0 ? (<div className="bg-white rounded-lg text-gray-600 font-bold mt-4 w-full h-[150px] grid place-items-center">No saved Jobs yet</div>) : (
                            <div className="grid grid-cols-2 gap-4">
                                {saved_jobs?.data.map((job: any, index: number) => (<EmployeeJobCard key={index} job={job} saved={true} />))}
                            </div>
                        )}
                    </div>
                </div>
